		...
		try {
			...
			for (String predName : toExtract.keySet()) {
				...
				if (arity == 1) {
					...
					String queryString = "SELECT ?x ?o WHERE { ?x a ?o}";
					...
					for (QuerySolution sol : solutions) {
						...
						if (!object.toString().contains(predName))
							continue;
						...
						if (node.isURIResource()) {
							...
							if (constant.isEmpty() || Character.isDigit(constant.charAt(0))
									|| !Character.isDefined(constant.charAt(0))) {
				...
				if (arity == 2) {
					...
					for (QuerySolution sol : solutions) {
						...
						if (!predicate.toString().contains(predName))
							continue; 
						String constant1 = "";
						String constant2 = "";
						...
						if (node1.isURIResource()) {
							...
							if (constant1.isEmpty() || Character.isDigit(constant1.charAt(0))
									|| !Character.isDefined(constant1.charAt(0))) {
						...
						if (node2.isURIResource()) {
							...
							if (constant2.isEmpty() || Character.isDigit(constant2.charAt(0))
									|| !Character.isDefined(constant2.charAt(0))) {
